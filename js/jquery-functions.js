$("document").ready(function () {
  ...
  function loadQuestion(questionId, noError) {
   ...
   function skipToEnd(message) {
    ...
    const error =
      currentLanguage === "greek"
        ? "Λυπούμαστε αλλά δεν δικαιούστε την αποζημίωση!"
        : "We are sorry but you are not entitled to compensation!";
  ...
  function retrieveAnswers() {
    ...
    if (allAnswers[4] === "1"){
      if(allAnswers[5] === "1" && allAnswers[6] === "3"){
        getEvidencesById(3)
      }
    }
    if (allAnswers[4] === "2"){
      if(allAnswers[5] === "1" && allAnswers[6] === "2"){
        getEvidencesById(3)
      }
    }
    if(allAnswers[4] === "3"){
    ...
    if(allAnswers[4] === "1"){
      if(allAnswers[5] === "1"){
        if(allAnswers[6] !== "3"){
          currentLanguage === "greek"
          ? setResult(
              "Συγγνώμη δεν δικαιούστε την αποζημίωση."
            )
          : setResult(
              "Sorry you are not eligible for compensation."
            );
        }
      }
    }
    ...
    if(allAnswers[4] === "3"){
      if(allAnswers[5] === "1"){
        if(allAnswers[6] === "1"){
          ...
          currentLanguage === "greek"
          ? setResult(
            "Δικαιούστε αποζημίωση μόνο για φακούς επαφής."
          )
          : setResult(
            "You are only entitled to compensation for contact lenses."
          );
          }
        }
      }
